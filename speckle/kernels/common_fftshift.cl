__kernel void execute(__global float* primary,
	...
	// i and j have range (0,N), (0,M), respectively.
	// for this problem however it is more natural to operate in the range
	// (-N/2,N/2);(-M/2,M/2)
	int x = (i+N/2)%N-N/2;
	int y = (j+N/2)%N-N/2;
	float fx = (float) x; // convert coordinates to float
	float fy = (float) y;
