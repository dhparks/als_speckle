...
def unwrap_plan(r,R,center,max_angle=None):
    ...
    if max_angle == None: max_angle = 2*numpy.pi
    ...
    x0Table = numpy.ravel(x0Table)
    y0Table = numpy.ravel(y0Table)
    
    # combine into the plan array
    # Plan format:
    # [[y0,y1,y2...R],[x0,x1,x2...r]]
    Plan = numpy.zeros((2,N+1),float)
    Plan[0,:-1] = y0Table
    Plan[1,:-1] = x0Table
...
def wrap_plan(r,R,max_angle=None):
    ...
    Optional input:
    max_angle: if when unwrapping you used a max_angle other than the default
        value of 2*pi it should be resupplied here. radians.
    ...
    yArray,xArray = numpy.indices((2*R1,2*R1),float)-R1
    RArray = numpy.sqrt(yArray**2+xArray**2)-R2
    PhiArray = numpy.mod(numpy.angle(xArray+complex(0,1)*yArray)+2*numpy.pi,2*numpy.pi)
    PhiArray *= 1./PhiArray.max()*int(2*numpy.pi*R-1)

    # put them in the plan
    Plan = numpy.zeros((2,len(yArray),len(yArray[0])),float)
    Plan[0] = RArray
    Plan[1] = PhiArray
...
def unwrap(array,plan,interpolation_order=3):
    ...
    plan or (r, R, center): Either a description of unwrapping, obtained by
        invoking unwrap_plan(), or the 3-tuple (r, R, center).  The latter
        method is useful if you only need to call unwrap() once.
    ...
    returns:
    unwrapped_array: the unwrapped array. The size of this is (R-r,2*pi*R)
    """
    import scipy.ndimage

    assert isinstance(array,numpy.ndarray),  "input data must be ndarray"
    assert interpolation_order in range(6), "interpolation order must be 0-5"
    if not isinstance(plan,numpy.ndarray):
...
def wrap(array,plan,interpolation_order=3):
    ...
    with all the real work being done in generating the plan. The plan can also
    be a 2-tuple of the inner/outer radii (r, R).
