...
def unwrap_plan(r, R, center, columns=None, modulo=None, target='cpu'):
    ...
    arguments:
        ...
        columns - number of columns in the unwrapped array. By default, the
            data is not resampled after unwrapping and the number of columns is
    ...
    assert isinstance(R, (float, int)), "R must be int or float"
    assert isinstance(center, (list, tuple, set)) and len(center) == 2,\
    "center must be 2-tuple"
    ...
    if columns == None:
        ...
        ucols = columns
    r = np.indices((uR-ur, ucols), float)[0]
...
def wrap_plan(r, R):
    """ Generate a plan to rewrap an array from polar coordinates into
    cartesian coordinates. Rewrapped array will be returned with coordinate
    center at the array center.
    ...
    returns:
        plan - a ndarray plan of coordinate wrapping map.
            To be used in wrap().
    """
    ...
    y_array, x_array = np.indices((2*R1, 2*R1), float)-R1
    r_array = np.sqrt(y_array**2+x_array**2)-R2
    phi_array = np.mod(np.angle(x_array+1j*y_array)+2*np.pi, 2*np.pi)
    phi_array *= 1./phi_array.max()*int(2*np.pi*R-1)

    # put them in the plan
    plan = np.zeros((2, len(y_array), len(y_array[0])), float)
    plan[0] = r_array
    plan[1] = phi_array
...
def unwrap(array, plan, interpolation_order=3, modulo=None, columns=None):
    ...
    arguments:
        ...
        plan or (r, R, center) - Either a description of unwrapping, obtained
            by invoking unwrap_plan(), or the 3-tuple (r, R, center).  The
            former method is useful if you only need to call unwrap() many times
            with the same 3-tuple values.
        interpolation_order - (optional) coordinate transformation
            interpolation order:
    ...
    assert interpolation_order < 6, "interpolation order must be 0-5"
    
    if columns != None:
        try:
            columns = int(columns)
        except TypeError:
            print "error casting columns=%s to int in unwrap"%columns
            exit()
    
    if not isinstance(plan, np.ndarray):
        assert len(plan) == 3,\
        "unwrap plan must be a len 2 list/tuple/set or a ndarray"
        
        r, R, center = plan
        plan = unwrap_plan(r, R, center, modulo=modulo, columns=columns)
    
    def _unwrap_help(data):
        """ Helper which maps and reshapes """
        uw = map_coordinates(data, plan, order=interpolation_order)
    ...
    assert plan[0].max() < array.shape[1] and plan[1].max() < array.shape[2],\
    "max unwrapped coordinates fall outside array.\ndid \
    you use the correct plan for this array size?"

    # unwrap each frame. reshape, then store in unwrapped
    unwrapped = np.array([_unwrap_help(frame) for frame in array])
...
def wrap(array, plan, interpolation_order=1):
    ...
    must be supplied. This is basically just a wrapper to
    scipy.ndimage.map_coordinates with all the real work being done in
    generating the plan. The plan can be a 2-tuple of the inner and outer radii
    (r, R).
...
def resize_plan(shape_in, shape_out, target='cpu'):
    ...
    arguments:
        ...
        target - (optional) either 'cpu' (default) or 'gpu' depending on
            which device does the calculation"""
    ...
    assert isinstance(shape_in, (tuple, list, np.ndarray)), \
    "shape_in must be interable (list, tuple, ndarray)"
    
    assert isinstance(shape_out, (tuple, list, np.ndarray)), \
    "shape_out must be iterable (list, tuple, ndarray)"
    ...
    assert isinstance(r_in, int) and isinstance(c_in, int), \
    "shape_in must be composed of integers"
    
    assert isinstance(r_out, int) and isinstance(c_out, int), \
    "shape_out must be composed of integers"
    ...
    # in scipy.nd_image.map_coordinates vs map_coords.cl.
...
def resize(array, plan, interpolation_order=3):
    ...
    arguments:
        ...
        plan or (r, R) - Either a description of unwrapping obtained by calling
            resize_plan(), or a 2-tuple (rows, cols) of new shape to
            interpolate.
        interpolation_order - coordinate transformation interpolation order.
            0 is nearest neighbor.
            1 is linear/bilinear
    ...
    if isinstance(plan, np.ndarray):
        ...
        assert len(plan) == 2, "plan must be len-2 tuple/set/list or ndarray"
        plan = resize_plan(array.shape, (plan[0], plan[1]))
    ...
    resized = scipy.ndimage.map_coordinates(array, plan,
