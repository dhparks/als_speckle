					  ...
					  __global float* plan_x, __global float* plan_y, int interpolation_order)
{
	...
	/* the algorithm is:
	 1. loop through each of the coordinates in OutputImage
	 2. the value of the first slice of Plan is the row in InputImage which should go there
	    the value of the second slice of Plan is the column in InputImage which should go there
	 3. get those values out of InputImage using the sampler, all the interpolation is provided by the framework
	 */
