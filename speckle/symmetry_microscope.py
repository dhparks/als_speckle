...
import numpy as np
import wrapping, gpu, sys, time, warnings
w = sys.stdout.write
common = gpu.common
...
class microscope(common):
    ...
    def __init__(self, gpu_info=None, force_cpu=False):
        ...
        # keep context, device, queue, platform, and kp in the
        ...
        if use_gpu:
            ...
            self.correl1 = gpu.build_kernel_file(self.context, self.device,
                                                 self.kp+'sm_correl_denoms.cl')
            self.correl2 = gpu.build_kernel_file(self.context, self.device,
                                                 self.kp+'sm_correl_norm.cl')
        else:
            self.compute_device = 'cpu'

        # these attributes change value in the load function. based on what
        # has been loaded, execution may be disallowed (ie, need an sample and
        # illumination function). 0 indicates no information, 1 partial
        # information, 2 complete information
        self.sample_state = 0
        ...
        # these class variables control analysis flow and which intermediates
        # are pulled in order to return as output. the keywords are the types
        # of output that can be added to the output dictionary. returnables is
        # the output dictionary.
        self.keywords = ('sliced', 'illuminated', 'speckle', 'blurred',
                         'unwrapped', 'correlated', 'spectrum', 'spectrum_ds',
                         'correlate_ds', 'rspeckle')
        self.returnables = {}
        ...
        # these variables get populated during load but are
        # instantiated here to avoid errors
        self.N1 = None
        self.N2 = None
        self.ur = None
        self.uR = None
        self.uwp_params = None
        
        # declare attributes in __init__ (its a style thing)
        self.resized_speckles = None
        self.c_out = None
        self.rpy = None
        self.N0a = None
        self.r_out = None
        ...
        self.rp = None
        self.N0b = None
        self.cached_spectra = None
        self.fft2 = None
        self.fftplan = None
        self.rpx = None
        self.N22 = None
        self.fftplan_speckle = None
        self.fftplan_correls = None
        self.fftplan_blur = None
        self.ifft2 = None
        self.despiked = None
        self.spectrumds = None
        self.far_field = None
        self.ipsf = None
        self.rowaverage = None
        self.speckles = None
        self.returnables_list = None
        self.sample = None
        self.N11 = None
        self.spectrumsum = None
        self.spectrumsumds = None
        self.upx = None
        self.upy = None
        self.blurred_f = None
        self.cached_spectra_ds = None
        self.sliced = None
        self.up = None

    def load(self, sample=None, illumination=None, ipsf=None, unwrap=None,
             returnables=None):
        ...
        Loadable items:
            ...
            sample -- (required) a numpy array representing the object over
                which the microscope runs. Physically, this should represent
                the transmission function in the Born approximation.
        ...
        def _load_sample(sample):
            ...
            def _need_new(sample):
                """ Helper: check if reallocation needed"""
                a = self.sample_state == 2
                try:
                    b = self.sample.shape == sample.shape
                except NameError:
                    b = False
                return (a and b) or (not a)
                
            if _need_new(sample):
                self.sample = self._allocate(sample.shape, np.complex64,
                                             name='sample')
        ...
        def _load_illumination(illumination):
            ...
            """ Load the illumination function and make fft plans """
            
            def _need_new(illumination):
                """ Helper: check if re-allocation is necessary """
                a = self.illumination_state == 2
                try:
                    b = self.illumination.shape != illumination.shape
                except NameError:
                    b = False
            ...
            if _need_new(illumination):
                self.illumination = self._allocate(s, d, name='illumination')
            self.illumination = self._set(illumination, self.illumination)
              
            # allocate memory for all the buffers of same size and shape
            # as illumination
            ...
            if use_gpu:
                self.fftplan_speckle = fft_plan((self.N1, self.N1), \
                    queue=self.queue)
            else:
                self.fftplan_speckle = None
                if have_fftw:
        ...
        def _load_ipsf(ipsf):
            """ Check the ipsf and allocate memory for blurry speckles """
            ...
            b1 = self.ipsf_state == 2 and s != ipsf.shape
            b2 = self.ipsf_state != 2
            if b1 or b2:
        ...
        def _load_unwrap(unwrap):
            """ Check the unwrap spec and create those self items
            which depend on the number of rows:
                self.ur
                self.uR
                self.spectrum
                self.spectrumds
            ...
            assert isinstance(unwrap, (list, tuple, np.ndarray)), \
            "unwrap must be iterable type"
            ...
            if use_gpu:
                assert self.rows%16 == 0, \
                "uR-ur must be divisible by 16 for gpu speed reasons"
            ...
            self.spectrum_buffer = self._allocate_local(16*16*8,
                                                        'spectrum_buffer')
            
            self.unwrap_state = 1
            
        def _load_returnables(returnables):
            """ Check the returnables spec and create self.returnables_list """
        
            # check types
            assert isinstance(returnables, (list, tuple)), \
            "returnables must be list or tuple"
            
            assert all([isinstance(r, str) for r in returnables]), \
            "all elements of returnables must be strings"
        
            self.returnables_list = []
            for r in returnables:
                if r not in self.keywords:
                    sys.stdout.write("requested returnable %s is unrecognized \
                                     and will be ignored\n"%r)
                else:
        ...
        def _make_resize_plan():
            ...
            self.resized_speckles = self._allocate(s2, np.complex64,
                                                   name='resized_speckles')

        def _make_unwrap_plan():
            """ make the unified unwrap/resize plan """
            
            if self.uR > self.N2/2:
                warnings.warn('\nThe supplied unwrap outer radius larger \
                              than the size of the speckle to unwrap.\nSetting\
                              outer radius to largest allowable value.\n')
                self.uR = self.N2/2
                self.rows = self.uR-self.uR
                if use_gpu:
                    assert self.rows%16 == 0, \
                    "uR-ur must be divisble by 16 for gpu speed reasons"
            
            uplan = wrapping.unwrap_plan(self.ur, self.uR, (0, 0), target='gpu')
            ...
            if not use_gpu:
                ...
                self.up = np.array([np.append(upy, self.uR),
                                    np.append(upx, self.ur)])
            ...
            self.rowaverage = self._allocate((self.rows,), d2, \
                name='rowaverage')
                
            # write down the parameters for this plan so that we don't have to
            # remake it on every re-load of sample or ipsf or illumination
            self.uwp_params = (self.ur, self.uR, self.N2)
            
            # correlation plan for gpu
            if use_gpu:
                self.fftplan_correls = fft_plan((512, ), queue=self.queue)
                
            self.unwrap_state = 2
        
        if returnables == None:
            returnables = ('spectrum_ds',)
        ...
        if unwrap != None:
            _load_unwrap(unwrap)
        if returnables != None:
            _load_returnables(returnables)
        if illumination != None:
            _load_illumination(illumination)
        ...
        if self.illumination_state == 2:
            ...
            if self.ipsf_state == 2:
                ...
                b1 = self.N1 != self.N2
                b2 = ipsf != None or illumination != None
                if b1 and b2:
                    # have illumination and differently size coherence and
                    # at least one of them is being loaded RIGHT NOW
                    self.resize_speckles = True
                    _make_resize_plan()
                    
                if use_gpu:
                    self.fftplan_blur = fft_plan((self.N2, self.N2), \
                        np.complex64, queue=self.queue)
            
            # see if we need a new unwrap plan. we require an unwrap plan
            # before we can run  the microscope.
            b1 = self.unwrap_state > 0
            b2 = unwrap != None or illumination != None or ipsf != None
            b3 = self.uwp_params != (self.ur, self.uR, self.N2)
            if b1 and b2 and b3:
                _make_unwrap_plan()

        b1 = self.sample_state == 2
        b2 = self.illumination_state == 2
        b3 = self.unwrap_state == 2
        if b1 and b2 and b3:
            self.can_run_scan = True
     
    def run_on_site(self, y, x, cache_spectra=False):
        
        """ Run the symmetry microscope on the site of the object described by
        the raster coordinates dy, dx. Steps are:
        ...
        5. Cosine-decompose the autocorrelation
            (in reality, fft and take even columns)
        
        This function aggregates the more atomistic methods into a
        single function.
        
        arguments:
            dy, dx: roll coordinates used as follows:
                np.roll(np.roll(d,r0,axis=0),r1,axis=1)
        ...
        self._decompose_spectrum(cache_spectra=cache_spectra)
    ...
    def run_on_sites(self, coords):
        ...
        Coords should be:
            iterable(iterable(y,x), iterable(y,x), iterable(y,x)...)
        ...
        s = ((len(coords), self.rows, 128))
        d = np.complex64
        if 'spectrum' in self.returnables_list:
            self.cached_spectra = self._allocate(s, d, name='cached_spectra')
        ...
        for m, coord in enumerate(coords):
            ...
            if 'spectrum'    in self.returnables_list:
                if use_gpu:
                    ...
                    self.cached_spectra[m] = self.spectrum
            if 'spectrum_ds' in self.returnables_list:
                if use_gpu:
                    self._kexec('copy_to_buffer_f2', self.spectrumds,
                                self.cached_spectra_ds, m,
                                shape=(self.rows, 128))
                else:
        ...
        if 'spectrum' in self.returnables_list:
            self.returnables['spectrum'] = self.get(self.cached_spectra)
        if 'spectrum_ds' in self.returnables_list:
            self.returnables['spectrum_ds'] = self.get(self.cached_spectra_ds)
    ...
    def status(self):
        ...
        if self.ipsf_state == 2:
            sys.stdout.write("    * Because ipsf is specified, \
                             speckle will be blurred\n")
            
        if self.resize_speckles:
            sys.stdout.write("    * Because illlumination and \
                             ipsf are different sizes,\n")
            
            sys.stdout.write("      speckle will be resized \
                             before blurring.\n")
        
    def timings(self):
        """ User accessible method which prints the timings
        in the microscope. Used for optimization """
        mt = self.times['mastr']
        for key in self.times.keys():
            if key != 'mastr':
                t = self.times[key]
                s = "%s time:  %.3e (%.2f %%)\n"
                sys.stdout.write(s%(key, t, t/mt*100))
        sys.stdout.write("mastr time: %.3e\n"%mt)

    def _is_square_array(self, array):
        """ Check if array is square; used in loading """
    ...
    def _blur(self):
        ...
        if self.resize_speckles:
            to_blur = self.resized_speckles
        else:
            to_blur = self.speckles
        
        # perform the convolution
        if use_gpu:
            self.fftplan_blur.execute(data_in=to_blur.data,
                                      data_out=self.blurred.data)
            self._cl_mult(self.blurred, self.ipsf, self.blurred)
            self.fftplan_blur.execute(self.blurred.data, inverse=True)
        ...
        if 'blurred' in self.returnables_list:
            self.returnables['blurred'] = shift(self.get(self.blurred).real)
    ...
    def _decompose_spectrum(self, cache_spectra=False):
        ...
        if use_gpu:
            ...
            b1 = 'spectrum_ds' in self.returnables_list
            b2 = 'correlated_ds' in self.returnables_list
            if b1 or b2:
                self._cl_copy(self.unwrapped, self.despiked)
                time0 = time.time()
                self._kexec('despike', self.despiked, self.despiked,
                            np.int32(4), np.float32(5), shape=(self.rows,))
                time1 = time.time()
            ...
            for entry in entries:
                ...
                time5 = time.time()
                self.fftplan_correls.execute(entry[1].data, batch=self.rows)
                time3 = time.time()
                self._kexec('cosine_reduce', entry[1], entry[0],
                            self.spectrum_buffer, shape=(128, int(self.rows)),
                            local=(16, 16))
                time4 = time.time()
            ...
            if self.sumspectra:
                ...
                if 'spectrum_ds' in self.returnables:
                    self.spectrumsumds.__add__(self.spectrumds)
        ...
        if not cache_spectra:
            if 'spectrum' in self.returnables_list:
                self.returnables['spectrum'] = self.get(self.spectrum)
            if 'spectrum_ds' in self.returnables_list:
                self.returnables['spectrum_ds'] = self.get(self.spectrumds)
            
        if 'correlated_ds' in self.returnables_list:
            self.returnables['correlated_ds'] = self.get(self.despiked)    

    def _fft2(self, data_in, data_out, inverse=False, plan=None):
        """ Unified wrapper for fft which abstracts the CPU/GPU issue """
        ...
        if use_gpu:
            plan.execute(data_in=data_in.data, data_out=data_out.data,
                         inverse=inverse)
        else:
            if inverse:
                return np.fft.ifft2(data_in)
            else:
                return np.fft.fft2(data_in)
    ...
    def _make_speckle(self):
        ...
        def _illuminate():
            """ Helper: illuminate the sample """
            time0 = time.time()
            if use_gpu:
                self._cl_mult(self.sliced, self.illumination, self.illuminated)
            else:
                self.illuminated = self.sliced*self.illumination
            self.times['illum'] += time.time()-time0

        def _speckles():
            """ Helper: make speckles """
            time0 = time.time()
            if use_gpu:
                self._fft2(self.illuminated, self.far_field,
                           plan=self.fftplan_speckle)
                self._cl_abs(self.far_field, self.speckles,
                             square=True)
            else:
                far_field = self.fft2(self.illuminated)
                if have_numexpr:
                    self.speckles = numexpr.evaluate('abs(far_field)**2')
                else:
                    self.speckles = np.abs(self.far_field)**2
            self.times['spckl'] += time.time()-time0
                
        def _resize():
            """ Helper: resize speckles """
            time0 = time.time()
            if use_gpu:
                self._cl_map2d(self.speckles, self.resized_speckles,
                               self.rpx, self.rpy)
            else:
                self.resized_speckles = wrapping.unwrap(self.speckles, self.rp)
        ...
        if 'illuminated' in self.returnables_list:
            self.returnables['illuminated'] = self.get(self.illuminated)
            
        if 'speckle' in self.returnables_list:
            self.returnables['speckle'] = shift(self.get(self.speckles))
            
        if 'rspeckle' in self.returnables_list and self.resize_speckles:
            tmp = shift(self.get(self.resized_speckles))
            self.returnables['rspeckle'] = tmp
            
        if 'speckle_blocker' in self.returnables_list:
            tmp = shift(self.get(self.speckles))*self.blocker
            self.returnables['speckle_blocker'] = tmp
    ...
    def _rotational_correlation(self):
        ...
        def _row_ave():
            """ Helper: average each row (for normalization) """
            if use_gpu:
                ls = 2
                self.correl1.execute(self.queue, (self.rows,), (ls,),
                                     self.unwrapped.data, self.rowaverage.data,
                                     pyopencl.LocalMemory(ls*4))
        ...
        def _corr_norm():
            """ Helper: normalize the correlation """
            if use_gpu:
                self.correl2.execute(self.queue, (512, self.rows),
                                     (16, 16), self.unwrapped.data,
                                     self.rowaverage.data,
                                     pyopencl.LocalMemory(16*4),
                                     self.unwrapped.data)
        ...
        # calculate the autocorrelation of the rows using a 1d plan and
        # the batch command in pyfft
        if use_gpu:
        ...
        if 'correlated' in self.returnables_list:
            self.returnables['correlated'] = self.get(self.unwrapped).real
    ...
    def _slice_sample(self, top_row, left_col):
        ...
        if use_gpu:
            ...
            c = np.int32(top_row)
            d = np.int32(left_col)
            self._kexec('slice_view', self.sample, self.sliced,
                        a, b, c, d, shape=(self.N1, self.N1))
            # look into the built in opencl method for slicing a rectangle
 
        else:
            
            # for the cpu path, we roll and slicing using indexes. this
            # requires that the sample be at least as large as the illumination
            # in both axes
            ...
            nr = np.roll
            rolls = lambda d, r0, r1: nr(nr(d, -r0, axis=0), -r1, axis=1)
            assert s[0] >= i and s[1] >= i
            
            self.sliced = rolls(self.sample, top_row, left_col)
            self.sliced = self.sliced[s[0]/2-j:s[0]/2+j, s[1]/2-j:s[1]/2+j]
            
        if 'sample' in self.returnables_list:
            self.returnables['sample'] = self.get(self.sample).real
        if 'sliced' in self.returnables_list:
            self.returnables['sliced'] = self.get(self.sliced).real
    ...
    def _unwrap_speckle(self):
        ...
        if self.ipsf_state == 2:
            to_unwrap = self.blurred
        else:
            to_unwrap = self.speckles

        time0 = time.time()
        if use_gpu:
            self._cl_map2d(to_unwrap, self.unwrapped, self.upx,
                           self.upy, shape=self.upx.shape)
            self._cl_abs(self.unwrapped, self.unwrapped)
        else:
            self.unwrapped = wrapping.unwrap(to_unwrap.real,
                                             self.up, interpolation_order=1)
        
        self.times['unwrp'] += time.time()-time0
        
        if 'unwrapped' in self.returnables_list:
            self.returnables['unwrapped'] = self.get(self.unwrapped).real
