...
class generator():
    ...
    def __init__(self,gpuinfo,N,domains,alpha=0.5,interrupts=(),ggr=None):
        ...
        self.get_m0                = gpu.build_kernel(self.context, self.device, kernels.get_m0)
        self.replace_dc_component1 = gpu.build_kernel(self.context, self.device, kernels.replace_dc_component1)
        self.replace_dc_component2 = gpu.build_kernel(self.context, self.device, kernels.replace_dc_component2)
        self.envelope_rescale      = gpu.build_kernel(self.context, self.device, kernels.envelope_rescale)
        self.findwalls             = gpu.build_kernel(self.context, self.device, kernels.find_walls) # looks at the 8 nearest neighbors.
        ...
        if ggr != None:
            ...
            self.debug_want    = cl_array.empty(self.queue,(self.N,self.N),numpy.float32)
            self.debug_need    = cl_array.empty(self.queue,(self.N,self.N),numpy.float32)
            self.plan          = self.ggr_make_plan(m0,rate,0.02,50)
            self.target        = 0
    ...
    def ggr_make_plan(self,m0,rate,transition,finishing):
    ...
    def ggr_iteration(self,iteration):
        ...
        # enforce the recency condition to prevent domain splittings
        ...
        opt_out = fminbound(self.ggr_optimize_spa,-1,1,full_output=1)
