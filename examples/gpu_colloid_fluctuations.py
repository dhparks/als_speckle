...
def samples():
    ...
    """ Use the methods of random_walk to generate simulations of particles
    undergoing brownian motion. Results are saved in ball_path which is
    a subdirectory of save_path which is specified in the parameters."""
...
def symmetry_microscope(sm_instance):
    ...
    save_name = '%s/gpu spectra bd%s br%s bs%s pr%s maxval_%.4e.fits'%(analysis_path,params.density,params.ballradius,params.brownianstep,params.pinhole,maxval)
...
def get_particulars(sm_instance,record):
    ...
    Same code structure as symmetry_microscope() but only runs on a few sites
    and saves different output. This could be unified with symmetry_microscope()
    with a small increase in code complexity.
    """

    loaded = -1 # start with a nonsensical value to ensure a frame is loaded below
    sm_instance.set_returnables(params.candidate_returnables)
            ...
            for col in xcoords:
                if record_site == raster_site:
                    ...
                    for key in sm_instance.returnables.keys():
                        speckle.io.save('%s/gpu %s %s %s.png'%(analysis_path,frame,record_site,key),sm_instance.returnables[key],components='real')
                    raster_site += 1
...
def find_candidates():
    ...
    file = glob.glob('%s/gpu spectra bd%s br%s bs%s pr%s maxval*.fits'%(analysis_path,params.density,params.ballradius,params.brownianstep,params.pinhole))[0]
...
if params.run_microscope or params.find_candidates:
    ...
    # here is an instantiation form for the class where the various components
    # are loaded one at a time. these methods can be used to change the object
    # or the illumination function without totally restarting the class, for example.
    ...
    # here is an alternate instantiation form in which the parameters are set during instantiation.
    # internally, these two forms are basically equivalent although for the set_xxx example it is
    # important to set_object first, as many variables depend on the object size. with this
    # form, the set_xxx methods are still available (see symmetry_microscope()).
    """microscope = gc.gpu_microscope(gpuinfo,
            object=params.N,
            unwrap=(params.unwrap_r,params.unwrap_R),
    ...
    nx = len(xcoords)
    ny = len(ycoords)
    nq = microscope.uR-microscope.ur # there is the possibility that uR and ur changed to avoid NaN bugs on Nvidia+Apple
